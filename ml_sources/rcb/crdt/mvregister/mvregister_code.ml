...
let queries: (string, string aset) known_queries = map_insert "read" (fun m -> list_map (fun x -> fst x) m) (map_empty ())
let register_init addr rid = 
      crdt_init addr rid ser ((rel, rel01), rel01) queries stabilize
